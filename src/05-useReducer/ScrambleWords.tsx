...
import { getInitialiState, scrambleWordReducer } from './reducer/scrambleWordReducer';
import confetti from 'canvas-confetti';

export const ScrambleWords = () => {

    const [state, dispatch] = useReducer(scrambleWordReducer, getInitialiState());
    ...
    useEffect(() => {
        if (points === 0) return;{
            // Aquí puedes agregar la lógica que desees cuando el juego termine
            confetti({
                particleCount: 100,
                spread: 70,
                origin: { y: 0.6 }
    ...
    const handlePlayAgain = () => {
        dispatch({ type: 'START_NEW_GAME', payload: getInitialiState() });
    };
